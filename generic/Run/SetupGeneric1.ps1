...
if (-not $filesonly) {
    ...
    Invoke-RestMethod -Method Get -UseBasicParsing -Uri $sql2019url -OutFile 'temp\SQL2022-SSEI-Dev.exe'
    ...
    if (($null -ne $process.ExitCode) -and ($process.ExitCode -ne 0)) { Write-Host ('EXIT CODE '+$process.ExitCode) } else { Write-Host 'Success' }
    Write-Host 'Configuring SQL Server 2022 Dev Edition'
    ...
    Write-Host 'Removing SQL Server 2022 Dev Edition Install Files'
